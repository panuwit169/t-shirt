      ...
      <div class="container">
        <div class="title">
          <h2 class="filter-title" style="text-align:left">
            <span class="content" style="font-size:16px;letter-spacing:1px;background-color: #fff;padding: 20px;display: inline;"><strong>รถเข็นสินค้า</strong></span>
          </h2><br>
        </div>
        <?php
        if(!isset($_SESSION["intLine"])||$_SESSION["count"]==0)
        {
        	echo "<p>คุณยังไม่มีสินค้าในรถเข็น</p><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br>";
        }
        else
        {
        mysql_connect("$host","$user","$pass");
        ...
        <?php
        if($_GET['error']=='true'){
        echo '<div class="alert alert-warning" role="alert">ผิดพลาด! ปริมาณที่สินค้ามีไม่เพียงพอ สำหรับปริมาณที่สินค้าที่คุณกรอกเมื่อซักครู่</div>';
        }
        ?>
        <div class="col-xs-12 col-md-8" style="border:1px solid #999;padding:20px">
        <table class="table">
          <thead>
            <tr>
              <th></th>
          ...
          for($i=0;$i<=(int)$_SESSION["intLine"];$i++)
          {
            ...
            <tbody style="vertical-align: middle">
        	  <tr>
            <td class="text-center"><a href="delete.php?Line=<?php echo $i;?>"><span class="glyphicon glyphicon-remove"></span></a></td>
            <td class="text-center"><img src="ViewImage.php?id=<?php echo $objResult['product_id'];?>" width="100px"></td>
        		<td><?php echo $objResult["product_name"];?></td>
            <td><?php echo strtoupper($objResult["stock_size"]);?></td>
        		<td><?php echo "฿".$objResult["product_price"];?></td>
        		<td>
            <form action="update.php" method="POST">
              <div style="width:120px">
                <div class="form-group form-group-options">
                            <div id="<?php echo $i;?>" class="input-group input-group-option quantity-wrapper">

                                <span  class="input-group-addon input-group-addon-remove quantity-remove btn">
                                    <span class="glyphicon glyphicon-minus"></span>
                                </span>

                                <input id="<?php echo $i;?>inp" name="quantity<?php echo $i;?>"type="text" value="<?php echo $_SESSION["strQty"][$i];?>" name="option[]" class="form-control quantity-count" placeholder="1">
        ...
        <input type="submit" value="อัพเดทรถเข็น" class="btn btn-default pull-right">
      </form>
        <a href="../product/product.php"><button class="btn btn-default">เลือกซื้อสินค้าต่อ</button></a>
      </div>
              ...
              <div class="panel-body">
                <table class="table">
                  <tbody>
                    ...
                    <tr>
                      <td>การจัดการและขนส่ง (ส่งแบบ EMS (กรุงเทพและปริมณฑล 1-2 วัน, ต่างจังหวัด 2-3 วัน))</td>
                      <td>฿50.00</td>
                    </tr>
                    <tr>
                ...
                <?php
                	...
                	{
                ?>
                	<center><a href="checkout.php"><button class="btn btn-success" style="margin:20px">เข้าสู่ขั้นตอนการชำระเงิน</button></a></center>
                <?php
                	}
                  mysql_close();
                }
